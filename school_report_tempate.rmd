...
school_dat <- hbsc2022[hbsc2022$school_id == params$school,] |>
  mutate(
    health = if_else(health %in% c("Good", "Excellent"), 1, 0),
    sex = if_else(sex == 1, "Boys", "Girls"),
    grade = if_else(Grade == "Secondary 2", "S2", "S4")
  ) |>
  pivot_longer(c(sex, grade), names_to = "div", values_to = "class") |>
  filter(!is.na(class)) |>
  group_by(class, div) |> 
    mutate(div = factor(div, levels = c("sex", "grade")),
         class = factor(class, levels = c("Girls", "Boys", "S2", "S4")))
