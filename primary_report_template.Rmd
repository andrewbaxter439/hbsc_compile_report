...
params:
  ...
  school: 
    ...
    input: select
    value: "020"
    choices: ["024","066","063","071","062","072","036", "020", "073", "013", "061", "048"]
...
school_dat <- hbsc2022[hbsc2022$SCHOOL_number == params$school,] |>
  ...
  mutate(
    sex = factor(paste0(sex_P7, "s"), levels = c("Girls", "Boys")),
    grade = "P7"
  )
...
HBSC Scotland Study 2022

Health and Wellbeing survey results for
:::

::: {custom-style="Schooltitle"}
`r ifelse(params$school_name == "NA", params$school, params$school_name)`
:::
...
-   Generally, within the same figure, we provide results by gender (in blue). Results for all pupils will be shown in red when numbers are low.
...
![](images/primary_graph_explanator_v2.png){width="7.138888888888889in"}

### Your school's participation in the survey

::: {custom-style="FigDesc"}
Pupils in P7 classes in your school took part in the survey
during spring term 2022. Table 1 shows the number of pupils
participating by gender. `r if(missing_vals$missing_gender > 0) paste0(missing_vals$missing_gender, " pupil", ifelse(missing_vals$missing_gender > 1, "s", ""), " did not provide their gender.")`
:::

```{r tab_n_pupils, tab.cap="Overview of survey respondents", tab.id = "n_pupils", tab.cap.style = "Caption", eval = TRUE}
school_dat |>
  count(sex) |> 
  mutate(sex = if_else(is.na(sex), "Not declared", as.character(sex))) |> 
  pivot_wider(names_from = sex, values_from = n, values_fill = 0) |> 
  rename_with(~paste0("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;", .x), .cols = 1:2) |>
  knitr::kable()
```


# Your school's survey results
::: {custom-style="Heading 2 - no split"}
...
::: {custom-style="FigDesc"}
Pupils were asked to rate their own health. This figure shows the proportion of pupils reporting their health to be either 'excellent' or 'good'.
:::
...
school_dat |>
  bar_by_cat(mhc_multi, "Yes", .gender_split = params$gender_split,
             .censor = params$censor)
...
::: {custom-style="FigDesc"}
Pupils were asked ten questions about sleep quality, covering three different aspects: (i) bedtime behaviours (e.g. delaying going to bed), (ii) sleep efficiency (e.g. have trouble settling down, need help getting to sleep or when waking up during the night) and (iii) morning wakefulness (e.g. waking up feeling rested and alert). For each aspect of sleep, scores range from 1 to 6. A higher score indicates better sleep quality in that aspect.
:::
...
An individual's mental health and wellbeing is influenced by a wide variety of factors including environmental, social and individual factors and the interaction between these. Contextual factors may influence or be a consequence of mental health and also serve as a risk or protective factor. These factors operate at different levels: individual, family, learning environment, community and structural.^13^ Deprivation is a key risk factor for poor mental health; a systematic review showed that children and adolescents from socio-economically deprived backgrounds were two to three times more likely to develop mental health problems.^14^

In terms of promoting wellbeing, positive relationships with family, friends and school staff are consistently linked with health and wellbeing during adolescence. The school environment plays a key role, with perceived academic ability, teacher support and school satisfaction linked to levels of life satisfaction and happiness.^15^
...
school_dat |>
  bar_by_cat(leftout, c("Often", "Always"), .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(confidence, c("Often", "Always"), .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(happiness, c("I feel quite happy", "I feel very happy"), .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(Lonely_mostalways, "Lonely most or all of time", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(WHO_low, "Low mood/risk of depression", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(var = aboutright, success = "Yes", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
[2]: https://www.mind.org.uk/information-support/for-children-and-young-people
[3]: https://www.annafreud.org/on-my-mind/
[4]: https://mentalhealthfoundation.org/
[5]: https://www.barnardos.org.uk/what-we-do/supporting-young-people
...
```{r food_drink_gender, fig.cap="% pupils who consume food/drink daily"}
school_dat |> 
  bar_multiple_vars(
  list(
    fruits_2 = "Fruit",
...
school_dat |>
  bar_by_cat(breakfastwd, success = "Five days", .gender_split = params$gender_split,
             .censor = params$censor)
...
```{r family_meal, fig.cap="% pupils who eat a meal with family most days or every day"}

school_dat |>
  bar_by_cat(fmeal, c("Every day", "Most days"), .gender_split = params$gender_split,
             .censor = params$censor)
...
school_dat |>
  bar_by_cat(var = MVPA7, success = "7 days of 60 mins MVPA a week", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
The following figure shows the percentage of pupils who reported doing vigorous exercise at least 3 times a week in their leisure time.
:::


```{r activity_four_times, fig.cap="% pupils who do vigorous exercise at least 3 times per week in their leisure time"}
school_dat |>
  bar_by_cat(var = vigexfrq4, success = "4 times a week or more", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
```{r organised_activity, fig.cap="% pupils who do at least one organised activity a week"}
school_dat |>
  bar_by_cat(var = Org_act_wkly, success = " At least one activity weekly", .gender_split = params$gender_split,
             .censor = params$censor)
```


::: {custom-style="FigDesc"}
The following figure shows the types of activities that young people
undertake, at least once a week.
:::

```{r activity_type, fig.cap="% pupils who do this type of activity at least once a week"}
school_dat |> 
  bar_multiple_vars(
  varslist = list(
...
the past couple of months, at school and been a victim of bullying. Pupils were also asked how often they had cyberbullied someone else in the past couple of months and how often they had been a victim of cyberbullying.

The following figure presents the percentage of pupils who were bullied or cyberbullied, or bullied others at least once or twice over the past couple of months.
:::


```{r all_bullying, fig.cap = "% of pupils who reported being bullied or cyberbullied, or bullying others at least once or twice over the past couple of months"}
...
```{r bullymost, results="asis"}
school_dat |>
  common_health_complaints(
    list(
      Bully_names = "Being called names, made fun of or teased",
      Bully_excluded = "Being left out, excluded or ignored",
      Bully_rumours = "Other pupils told lies or spread rumours about them",
      Bully_race = "Being bullied regarding their race",
      Bully_religion = "Being bullied regarding their religion"
      # Bully_hit = "[hit]"
    ),
    success = "Bullied 2-3 times+",
    group = "sex"
  )
```
...
Several studies have suggested that use of social media may have a positive effect on self-esteem, friendship closeness, social competence and empathy,^37^ though other studies have found that online social interactions are only beneficial when communicating with existing friends, rather than relationships with strangers which have been created online.^38^

The \#Sleepyteens project in Scotland found that children who were very emotionally invested in social media and used it the most had poorer sleep quality, lower self-esteem, and higher levels of anxiety and depression.^39^ Compulsive social media use has been linked to depression, poor sleep quality, low life satisfaction, anxiety and aggressive behaviour.^40,41^

The Royal College of Paediatrics and Child Health have produced guides for parents to help negotiate the right amount of screen time for their own children.^42^
...
school_dat |>
  bar_by_cat(var = smartphbed, success = " Has smartphone in bedroom", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(var = TVbed, success = " Has TV in bedroom", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(var = emconhigh1, success = "Intense contact", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_mean_multiple_vars(
    ...
    group = "sex",
    ymax = max(6, max(select(school_dat, ends_with("hr")), na.rm = TRUE)),
    ylab = "Hours",
    .gender_split = params$gender_split,
    .censor = params$censor
  )
...
A review carried out by Public Health Scotland^43^ looked at the possible positive and adverse consequences of the first lockdown on young people's development. In terms of family relationships, it found that young people may have experienced 'stressors' including poor parental wellbeing, changing family financial situations and limited access to services. However, most young people (aged 12-14 years old) enjoyed spending time with their family during lockdown.

Another important aspect impacted by the pandemic is mental health and wellbeing. Feelings of loneliness (particularly amongst girls) increased during the first lockdown ^44^. Girls have also been more likely to report lower levels of mental wellbeing and higher levels of anxiety than boys.^43-45^ Older adolescents also reported feeling concerned about the impact of lockdown on their education and future careers.^43^
...
```{r covid_girls, fig.cap="Perceived impact of the COVID-19 pandemic on GIRLS", fig.height=4.2, fig.width=6.4, eval = params$gender_split}
school_dat |>
  bar_diverging("Girls")
```


```{r covid_boys, fig.cap="Perceived impact of the COVID-19 pandemic on BOYS", fig.height=4.2, fig.width=6.4, eval = params$gender_split}
school_dat |>
  bar_diverging("Boys")
```

```{r covid_all, fig.cap="Perceived impact of the COVID-19 pandemic on all pupils", fig.height=4.2, fig.width=6.4, eval = !params$gender_split}
school_dat |>
  mutate(grade = "All pupils") |> 
  bar_diverging("All pupils")
```
...
Supportive family relationships are vital for adolescent development, socialisation, health and wellbeing.^46^ Good support leads to improved mental health and better educational outcomes.^47^ Family support reduces risky health behaviours in adolescents^48^ and provides a buffer against adverse life events and bullying, especially for girls.^49^ Family communication is also important for health, wellbeing and life satisfaction.^50^ Studies have shown that family communication is important for reducing risk behaviours such as illegal substance use, smoking, drinking, early sexual behaviour and suicidal behaviour.^51-53^

Another important source of support is that offered by peers, which is also linked to better health in adolescents.^54^ Support from classmates has a positive effect on mental wellbeing by helping to boost self-efficacy and building a sense of school community.^55^

The negative effects of adverse childhood experiences on mental and physical health across the lifespan can be reduced by having the support of a trusted adult in childhood. A review by NHS Scotland demonstrates the importance of having an adult that young people trust and can talk to, with young people reporting this having higher educational attainment, optimism and self-efficacy as well as lower internalising symptoms (depression and anxiety).^56^
...
school_dat |>
  bar_by_cat(var = easytalk_mfigure, success = "Easy to talk to mother or stepmother", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
```{r easy_talk_f, fig.cap="% pupils who say it's easy or very easy to speak to father figure"}
school_dat |>
  bar_by_cat(var = easytalk_Ffigure, success = "Easy to talk to father or stepfather", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
::: {custom-style="FigDesc"}
The HBSC survey includes the Family Support Scale. This consists of four questions about how much support the young person feels they get from their family, for example, listening to their problems and help with decision making. The figure shows the percentage of pupils who report high family support.
:::

```{r high_support, fig.cap="% pupils who report high family support"}
school_dat |>
  bar_by_cat(var = Family_support_scale_binary, success = "High family support", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(var = Peer_support_scale_binary, success = "High Peer support", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(var = Trusted_adult, success = "Yes, I always do", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
The school environment has many effects on pupil wellbeing and a positive school experience can bring many benefits in terms of both health and education. Students who feel they are able to handle their work, have good teacher support and are satisfied with school are more likely to report high life satisfaction.^57^ Good teacher support is linked to better mental health and lower use of alcohol, cigarettes and cannabis,^58^ and the positive link between teacher support and emotional wellbeing has been shown to be strong regardless of pupil demographics or perceived school performance.^59^ High classmate support is also linked to improved mental health,^60^ lower drunkenness, and for males is linked to lower rates of smoking.^61^

In contrast, high levels of school pressure are associated with higher levels of risky behaviour and poorer mental health.^62,63^ Low school satisfaction is linked to health behaviours such as smoking, alcohol and cannabis use, and gambling,^64,65^ as well as poorer self-rated health and increased physical and psychological symptoms.^66,67^
...
school_dat |>
  bar_by_cat(var = likesch_bitlot, success = "Like school a bit or a lot", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(var = school_stress, success = "some/a lot", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(var = Teacher_scale_cutoff, success = "High teacher support", .gender_split = params$gender_split,
             .censor = params$censor)
```

```{r high_class_support, fig.cap="% pupils who report high classmate support"}
school_dat |>
  bar_by_cat(var = stud_support_scale_cutoff, success = "High student support", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
39. Woods, H. C., & Scott, H. (2016). \#Sleepyteens: Social media use 
    in adolescence is associated with poor sleep quality, anxiety,
    depression and low self-esteem. *Journal of Adolescence*, 51, 
    41--49.

40. Boniel-Nissim, M., Tabak, I., Mazur, J., Borraccino, A., Brooks, 
...
41. Ko, C.-H., Yen, J.-Y., Chen, C.-S., Yeh, Y.-C., & Yen, C.-F. 
...
44. Edinburgh University (2021). TeenCovidLife Survey 2 - General 
...
47. Wilkinson, R. B. (2004). The Role of Parental and Peer Attachment 
...
53. Zaborskis, A., Sirvyte, D., & Zemaitiene, N. (2016). Prevalence 
...
64. Tennant, J. E., Demaray, M. K., Malecki, C. K., Terry, M. N., 
