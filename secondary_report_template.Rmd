...
school_dat <- hbsc2022[hbsc2022$SCHOOL_number == params$school,] |>
  ...
  mutate(
    grade = case_when(Grade == "Secondary 2" ~ "S2",
                      Grade == "Secondary 4" ~ "S4",
                      TRUE ~ "not supplied"),
    grade = factor(grade, levels = c("S2", "S4")),
    sex = factor(paste0(sex, "s"), levels = c("Girls", "Boys"))
  )

multi_grades <- "S2" %in% school_dat$grade & "S4" %in% school_dat$grade & all(count(school_dat, grade)$n > 7)
...
-   Generally, within the same figure, we provide results by gender (in blue) and also by year group (in green). Results for all pupils will be shown in red when numbers are low.
...
![](images/graph_explanator_v2.png){width="7.138888888888889in"}
...
school_dat |>
  ...
  mutate(sex = if_else(is.na(sex), "Other or not declared", as.character(sex)),
         grade = if_else(is.na(grade), "Not declared", as.character(grade))) |> 
  pivot_wider(names_from = grade, values_from = n, values_fill = 0) |> 
  rename(" " = sex) |> 
  adorn_totals(c("row", "col")) |> 
  rename_with(~paste0("&nbsp;&nbsp;&nbsp;", .x), .cols = 2:3)  |>
  knitr::kable()
```
...
```{r mean_sleep_grade, fig.cap="Pupils' mean sleep quality scores, by year group", eval=multi_grades}
school_dat |>
  bar_mean_multiple_vars(
  list(
    SleepQual_GTB = "Bedtime behaviours",
...
school_dat |>
  bar_by_cat(var = gadclassification, success = "anxious",.gender_split = params$gender_split,
             .censor = params$censor)
```
...
school_dat |>
  bar_by_cat(var = vigexfrq3, success = "3 times a week or more", .gender_split = params$gender_split,
             .censor = params$censor)
```
...
```{r activity_type_grade, fig.cap="% pupils who do this type of activity at least once a week, by year group", eval = multi_grades}
school_dat |> 
  bar_multiple_vars(
  varslist = list(
    ls_teamsp_wk = "Team sports",
...
-   Other pupils made sexual jokes or gestures to them -- `r perc_success(school_dat, Bully_sexjoke, "Bullied 2-3 times+")`
-   Being called names, made fun of or teased -- `r perc_success(school_dat, Bully_names, "Bullied 2-3 times+")`
<!-- This one doesn't seem to have a variable? -->
<!-- -   Received mean comments about weight -- -->
-   Other pupils told lies or spread rumours about them -- `r perc_success(school_dat, Bully_rumours, "Bullied 2-3 times+")`
-   Being left out, excluded or ignored -- `r perc_success(school_dat, Bully_excluded, "Bullied 2-3 times+")`
...
Nonetheless, frequent and excessive drinking is associated with a range of problems such as future alcohol and drug use, academic problems, risky sexual behaviour, and various physical and emotional problems.^37-38^

Although nicotine vaping products (NVPs), or e-cigarettes’, are considered a lower risk to health than tobacco, the majority still contain nicotine. World Health Organisation (WHO) suggests that nicotine consumption by adolescents can have negative effects on brain development leading to learning and anxiety disorders.^39^  UK and Scottish government recommend these products are not used by children or young people and sale of products to under 18s is banned.
...
```{r test_substance_use_denoms, include=FALSE}

subs_use_vars <- list(
  drunk1_life = "Been drunk",
...
::: {custom-style="FigDesc"}
`r if(include_subs_split){"The figures show"} else {"This figure shows"}` the percentage of pupils who have used each substance at least once in their lifetime.
:::

```{r substance_combined, fig.cap="% pupils who reported substance use in their lifetime", eval=!include_subs_split}
school_dat |>
  bar_multiple_vars(
    subs_use_vars,
    success = c("drunk once or more", "Tried smoking", "Tried e-cigarette"),
...
```{r substance_gender, fig.cap="% pupils who reported substance use in their lifetime, by gender", eval=include_subs_split}
school_dat |>
  bar_multiple_vars(
    subs_use_vars,
    success = c("drunk once or more", "Tried smoking", "Tried e-cigarette"),
...
```{r substance_grade, fig.cap="% pupils who reported substance use in their lifetime, by year group", eval=include_subs_split & multi_grades}
school_dat |>
  bar_multiple_vars(
    subs_use_vars,
    success = c("drunk once or more", "Tried smoking", "Tried e-cigarette"),
...
```{r screen_use_grade, fig.cap="Mean time (hours) pupils spend per day using a screen, by year group", eval = multi_grades}
school_dat |>
  bar_mean_multiple_vars(
    list(
      sit_gamehr = "Gaming",
...
school_dat |>
  bar_by_cat(var = game_4day, success = "Gaming at least 4 days a week",.gender_split = params$gender_split,
             .censor = params$censor)
```


Pupils who gamed each week were asked to complete the Internet Gaming Disorder (IGD) Scale which distinguishes between 'normative' and 'problematic' gaming. Overall, `r school_dat |> filter(!is.na(problematicgame)) |>  perc_success(problematicgame, "Problematic gamer")` of all pupils who game reported problematic gaming.
...
```{r covid_s2, fig.cap="Perceived impact of the COVID-19 pandemic on S2 pupils", eval=multi_grades, fig.height=4.2, fig.width=6.4}
school_dat |>
  bar_diverging("S2")
```


```{r covid_s4, fig.cap="Perceived impact of the COVID-19 pandemic on S4 pupils", eval=multi_grades, fig.height=4.2, fig.width=6.4}
school_dat |>
  bar_diverging("S4")
```


```{r covid_all, fig.cap="Perceived impact of the COVID-19 pandemic on all pupils", eval=(!multi_grades|!params$gender_split), fig.height=4.2, fig.width=6.4}
school_dat |>
...
38. Eaton, D. K., Kann, L., Kinchen, S., Ross, J., Hawkins, J.,
    Harris, W. A., ... Wechsler, H. (2006). Youth Risk Behavior
    Surveillance---United States, 2005. *Journal of School Health*, 
    76(7), 353--372.
    
39. World Health Organization (2021) WHO Report on the Global Tobacco Epidemic, 2021: Addressing new and emerging products. WHO, Geneva.
